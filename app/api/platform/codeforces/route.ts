...
/**
 * Codeforces Platform Profile API Route (Thin Adapter)
 * Delegates external fetching and normalization to PlatformIntegrationService.
 */
export async function POST(req: NextRequest) {
  try {
    const body = await req.json().catch(() => ({}));
    const { status, body: responseBody } = await PlatformIntegrationService.fetchCodeforcesProfile(body?.username);
