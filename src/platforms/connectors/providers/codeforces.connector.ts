/**
 * Real Codeforces Platform Connector
 * Sources profile, contest, and submission metrics from server route /api/platform/codeforces.
...
export class CodeforcesConnector implements IPlatformConnector {
  public readonly platformId: string = 'codeforces';
  public readonly name: string = 'Codeforces Official API Connector';
  ...
  private async fetchServerData(handle: string): Promise<{ profile: PlatformUserProfile | null; contestHistory: ContestRecord[] }> {
    ...
    try {
      const res = await fetch('/api/platform/codeforces', {
        method: 'POST',
      ...
      console.error('[CodeforcesConnector] Error fetching server proxy data:', err);
      return { profile: null, contestHistory: [] };
    }
  }
