/**
 * Real LeetCode Platform Connector
 * Sources profile, contest, and submission metrics from server route /api/platform/leetcode.
...
export class LeetCodeConnector implements IPlatformConnector {
  public readonly platformId: string = 'leetcode';
  public readonly name: string = 'LeetCode Official GraphQL Connector';
  ...
  private async fetchServerData(handle: string): Promise<{ profile: PlatformUserProfile | null; contestHistory: ContestRecord[] }> {
    ...
    try {
      const res = await fetch('/api/platform/leetcode', {
        method: 'POST',
      ...
      console.error('[LeetCodeConnector] Error fetching server proxy data:', err);
      return { profile: null, contestHistory: [] };
    }
  }
